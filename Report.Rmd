...
XGBoost achieved the lowest $RMSE$ of 1.56 as well as the $R²$ closest to 1, indicating that it is really the best performing model in our case. The relatively low error suggests that XGBoost can accurately predict tree percentage based on the input features. Random Forest model also performed quite well considering its low $RMSE$ of 2.51 and $R²$ very close to 1. Linear Model had decent performance once again and Neural Network Model performed the worst out of all, which we previously observed.
