...
Once the files were loaded into R, they were rasterized using the [`terra::rast`](https://cran.r-project.org/web/packages/terra/terra.pdf#Rfn.rast.1) [@terraRastCite] from `terra` package[@terraCite] function, preparing them for further geo-spatial operations.
...
To ensure that the analysis focuses solely on Central Europe, the raster data was cropped ([`terra::crop`](https://cran.r-project.org/web/packages/terra/terra.pdf#Rfn.crop.1) [@terraCropCite]) and masked ([`terra::mask`](https://cran.r-project.org/web/packages/terra/terra.pdf#Rfn.mask.1) [@terraMaskCite]) based on the spatial boundaries of the selected countries. The [`terra::aggregate`](https://cran.r-project.org/web/packages/terra/terra.pdf#Rfn.aggregate.1) [@terraAggregateCite] function reduces the resolution for computational efficiency, while the [`terra::project`](https://cran.r-project.org/web/packages/terra/terra.pdf#Rfn.project.1) [@terraProjectCite] function ensures that the rasters have a consistent coordinate reference system (CRS). The resulting rasters were saved in `GeoTIFF` format, enabling the creation of virtual raster layers. These virtual layers are more computationally efficient and better suited for handling large-scale geospatial data during analysis.
...
To analyze land cover data across different years, we used the [`exact_extract`](https://cran.r-project.org/web/packages/exactextractr/exactextractr.pdf#Rfn.exact.Rul.extract.1) [@extractextractFuncCite] function from the `exactextractr` [@extractrCite] package. This function calculates the total area (in km²) of each administrative region in Central Europe and determines what percentage of that area is covered by specific land cover types, such as water, trees, and built areas. The function `calculate_coverage_percentage` was created to perform these calculations for each region and store the results in separate data frames for each year. Each data frame contains the total area of a region and the percentage of that region covered by various land cover types.
...
*GFED Global Monthly Emissions [2002 - 2023]* [@globalMonthlyEmissions] data set can also be found on [Global Wildfire Information System](https://gwis.jrc.ec.europa.eu/apps/country.profile/downloads) and can be downloaded following this [link](https://effis-gwis-cms.s3.eu-west-1.amazonaws.com/apps/country.profile/emission_gfed_full_2002_2023.zip). provides monthly emissions data for various pollutants, such as CO2, CO, PM25, and CH4, across the globe. This dataset contains emission observations for first-level administrative regions of each country in the world.
...
![Clumulative Tree Cover Loss in Central European Countries Over the course of 2001 to 2023](Images/treelosmapp.png)
...
Spatial autocorrelation describes the extent to which a variable is correlated with itself across space. A similar concept is found in [Tobler's First Law of Geography](https://www.jstor.org/stable/143141?origin=crossref) [@firstLawOfGeography], which states that *"everything is related to everything else, but near things are more related than distant things".*
...
In order to gain first impressions about spatial autocorrelation of `tree_percentage` over the years, we will examine Moran's I scatterplots for each year. The [`moran.plot()`](https://cran.r-project.org/web/packages/spdep/spdep.pdf#Rfn.moran.plot.1) [@moranPlotCite] function is used to create these scatterplots, which visualize the relationship between each observation and its spatially lagged values (i.e., the weighted average of its neighboring values). Using the [`nb2listw()`](https://cran.r-project.org/web/packages/spdep/spdep.pdf#Rfn.nb2listw.1) [@nb2listCite] function from the `spdep` [@spdepCite] package, we compute the spatial weights that help highlight this relationship and that are also used by other functions of `spdep` package which are going to be discussed in further work.
...
As we have gained an idea about spatial autocorrelation of our variable of interest, we aim to test our hypothesis of positive spatial autocorrelation. At the first step we are going to employ function [`moran.test()`](https://cran.r-project.org/web/packages/spdep/spdep.pdf#Rfn.moran.test.1) [@moranTestCite] of the `spdep` [@spdepCite] package to test spatial autocorrelation using Moran's I. this function allows us to set argument called `alternative` which denotes alternative hypothesis and can be set to `greater`, `less` or `two.sided`. For this test we are going to set `alternative="greater"` which specifies hypothesis as follows:
...
To further assess the significance, we can utilize a Monte Carlo approach. This method creates random patterns by reassigning the observed values among fixed areas and calculates Moran's I for each randomized pattern. The p-values are then computed as the proportion of simulated values that are as extreme or more extreme than the observed statistic, thereby providing a randomization-based significance test. The `spdep` package provides the [`moran.mc()`](https://cran.r-project.org/web/packages/spdep/spdep.pdf#Rfn.moran.mc.1) [@moranmcCite] function to perform this Monte Carlo simulation.

moranmcCiteWe are going to calculate Moran's I statistic using `moran.mc()` employing 999 simulations and extract the results into a data frame in order to make plots that reveal a pattern and let us decide on our hypothesis:
...
The Global Moran's I provides a measure of spatial autocorrelation for the entire study area. However, it is often useful to analyze spatial clustering at a more localized level using [Local Indicators of Spatial Association (LISA)](https://onlinelibrary.wiley.com/doi/10.1111/j.1538-4632.1995.tb00338.x) [@anselin1995lisa]. LISA measures the degree of spatial clustering around each observation. One key property of LISA is that the sum of the local Moran's I values across all regions is proportional to the global Moran's I. This allows us to break the global statistic into local components, offering more granular insights.
...
In this section, we calculate the **Local Moran's I** statistic for each year from 2018 to 2023 to explore the local spatial autocorrelation patterns for the variable `tree_percentage`. We use the `localmoran()` [@localMoranCite] function from the `spdep` [@spdepCite] package, which computes Local Moran's I based on the spatial weights for the data. we again set alternative hypothesis to "greater", where:
...
We will visualize results for each year using `tmap` [@tmapCite] package. Areas in which p-value is less than the significance level $0.05$ (in other words areas where z-scores are higher than `qnorm(0.95) = 1.65`) are the ones for which we reject null hypothesis $H_0$ and conclude they present positive spatial autocorrelation. We will visualize Tree percentage of each year and Local Moran's I statistic as well, to also observe how these changed over time.

![Moran's I Statistics for 2018.](Images/lmi2018.png)

![Moran's I Statistics for 2019.](Images/lmi2019.png)

![Moran's I Statistics for 2020.](Images/lmi2020.png)

![Moran's I Statistics for 2021.](Images/lmi2021.png)

![Moran's I Statistics for 2022.](Images/lmi2022.png)

![Moran's I Statistics for 2023.](Images/lmi2023.png)
...
![Moran's I Scatterplot showing the scaled values against spatially lagged values for tree percentage in 2018. States which represent outliers are marked in blue.](Images/Rplot18.png)
...
The correlation between the tree percentage values and their lagged values is computed using **Pearson correlation** [@pearsonsCorrCite]. Pearson correlation quantifies the linear relationship between two variables---in this case, the tree percentage for a given year and the tree percentage from the previous year (lagged values). The formula for Pearson correlation between two variables $X$ (tree percentage) and $Y$ (lagged tree percentage) is:
...
**Important note:** Before we dive into intepreting autocorrelation plot from above, it's important to acknowledge that the correlation is calculated based on only six values per region (tree percentage values from 2018 to 2023), which means the sample size is relatively small. As such, any conclusions drawn from this analysis must be interpreted with caution. The observed correlations may not be statistically robust due to the limited data points, and performing traditional statistical tests (e.g., significance testing) may not yield meaningful results. Permutation tests, which are often used to assess the significance of correlations, would also be inappropriate here due to the time-series structure of the data, as the observations are not exchangeable across time.
...
**Note:** As already noted for temporal autocorrelation, following results are to be taken with caution, due to uncertainty arising with small sample size.
...
|                                          |                                               |                                           |
|------------------------|-----------------------|--------------------------|
| ![](Images/treecropcor.png){width="206"} | ![](Images/treerangelandcor.png){width="216"} | ![](Images/treebuiltcor.png){width="207"} |
...
We aim to model and predict tree percentage based on multiple land-use and pollution-related variables. Understanding how different variables such as built area percentage, crops percentage, rangeland percentage, and pollution levels influence tree coverage is essential to guide environmental policies and land-use decisions. **It is important to note that this model is explanatory rather than predictive. This means that all variables are observed at the same point in time, and the primary goal of the analysis is to explain the relationships between the variables rather than to forecast future values.** While this is a valid approach for exploring relationships, it does not imply that the model is capable of predicting future tree coverage based on past observations of land-use variables.

As a critical step before model fitting, we examine the correlation matrix to explore the relationships between the selected land-use and pollution-related variables. This helps us understand how these variables interact and their potential influence on **tree percentage**.

Through earlier correlation analysis between **tree percentage** and several variables such as **built area percentage** and **crops percentage**, we gained some deep insights and have seen that tree percentage in the most of the Central European areas correlates heavily negative with these variables. To visualize the correlation between variables, we are going to use `corrplot` function from R package of the same name [@corrplot2024].

```{r eval=FALSE, include=TRUE}
corr_data <- time_series_data %>%
  select(tree_percentage, built_area_percentage, crops_percentage, 
         rangeland_percentage, water_percentage,
         flooded_vegetation_percentage, bare_ground_percentage, 
         snow_ice_percentage, burned_area, CO2_total,
...
![Correlation Matrix of variables used for model fitting.](Images/Rplot20.png)

Now, this broader correlation plot offers additional insights into the relationships between not only **tree percentage** and other variables, but also between the land-use and pollution metrics themselves. Pollution-related variables, including **BC_total**, **SO2_total**, and **CH4_total**, show strong correlations with each other, suggesting that these factors often co-occur due to shared sources of emissions. All of these pollution variables are also highly positively correlated with `burned_area` as those pollutants were measured in in years where wild fires have taken place in Central European regions.

### Model Fitting Approaches & Train-Test Split & Performance Evaluation

We began by fitting several models to predict tree percentage, each of which was evaluated for performance. Performance is evaluated by splitting the data into training and test sets. The training data consists of observations from 2018 to 2022, while the test data includes observations from the year 2023. This allows us to check how well the model fits unseen data, offering a measure of its robustness.
...
First we tried to fit a simple linear model using `lm` function form `stats` [@statsCite] package where tree percentage is assumed to have a linear relationship with land-use and pollution-related variables.

```{r eval=FALSE, include=TRUE}
lm_model <- lm(tree_percentage ~ built_area_percentage + crops_percentage 
               + rangeland_percentage + water_percentage
               + flooded_vegetation_percentage + bare_ground_percentage 
...
Neural networks are a class of machine learning models inspired by the structure and function of the human brain, making them highly effective at capturing complex, non-linear relationships in data. In R, package `nnet` [@nnetCite] allows users to train such neural network models. Given the diverse and multi-dimensional nature of the dataset, which includes variables like built area percentage, crops percentage, and pollution levels, a neural network was a promising candidate for predicting tree percentage. By employing multiple hidden layers and non-linear transformations, the model has the potential to uncover intricate interactions between these variables that traditional linear models might overlook.
...
xgb_model <- xgboost(data = train_matrix, label = train_data$tree_percentage, 
...
![**Actual vs. Predicted Tree Percentages from XGBoost.** XGBoost shows very well fitted alignment between the predicted and actual tree percentages, as the points tightly follow the ideal line, which represents the ideal $x=y$ relationship. The distances of points to the ideal line are minimal and there don't seem to be any huge deviations from the line, or points with higher or lower tree percentage heavly clustered together further from the ideal line. Model appears to have effectively captured underlying relationships between the tree percentage and predictor variables and there is very little deviation between actual and predicted values.](Images/testData2023xgboost.png)

### Model Comparison

#### RMSE Metric

We can use $RMSE$ (Root Mean Squared Error), to evaluate models performances. $RMSE$ measures the average magnitude of the errors between predicted and actual values.
...
**Low** $RMSE$ indicates that the predicted values are very close to the actual values, which means the model is performing well, where as **high** $RMSE$ suggests that there are significant errors in the model's predictions and that the model may not be capturing the underlying patterns in data well.
...
![$RMSE$ Results.](Images/Screenshot%20from%202024-10-17%2013-20-07.png){width="414"}

XGBoost achieved the lowest $RMSE$ of 1.56, indicating that it is really the best performing model in our case. The relatively low error suggests that XGBoost can accurately predict tree percentage based on the input features. Random Forest model also performed quite well considering its low $RMSE$ of 2.51. Linear Model had decent performance once again and Neural Network Model performed the worst out of all, which we previously observed.

#### R-Squared Metric

We will now compare the four models once again to ensure that XGBoost is indeed the best fitting model. To do this, we will utilize the R-squared metric, also known as the coefficient of determination, which measures how well the model's predictions align with the actual data and is known to be more informative in regression analysis evaluation than other metrics [@chicco2021r_squared] (SMAPE, MAE, MAPE, MSE and RMSE).

$$
R^2 = 1 - \frac{\sum_{i=1}^{n} (y_i - \hat{y}_i)^2}{\sum_{i=1}^{n} (y_i - \bar{y})^2}
$$

Where $y_i$ are the **actual** observed values (tree percentage), $\hat{y}_i$ are the **predicted** values from the model, $\bar{y}$ is the **mean** of the actual values and $n$ is the number of observations.

This formula essentially compares the total variance in the actual values to the variance in the residuals (errors), with $R^2$ representing the proportion of variance explained by the model. The $R²$ value ranges from 0 to 1, where 1 indicates that the model explains all the variability in the response variable, while 0 means that the model fails to explain any of the variability. This comparison will provide a clearer understanding of each model's performance.
...
![$R²$ Results.](Images/Screenshot%20from%202024-10-17%2013-06-31.png)

The results show that XGBoost is indeed the best-performing model compared to the others tested, as demonstrated by its R-squared value of nearly 1, indicating a near-perfect fit. This suggests that XGBoost is able to capture almost all the variability in the tree percentage, making it the most reliable for predictions in our dataset. Random Forest model also indicates high R-squared metric but XGBoost stil indicates slightly higher R-squared value and better model fitting.
...
simulate_tree_percentage <- function(test_data, model, 
...
![Predictor Optimization Results by Region.](Images/Screenshot%20from%202024-10-20%2014-25-15.png)

As seen from the output, **94 regions** experienced improvements in tree percentage after the simulated changes in predictor variables. The **average yearly improvement** in regions where tree percentage increase was simulated was 3.06%. On the other hand **14 regions** experienced slight decline in tree cover percentage where yearly decline was -0.919%.

In conclusion, land use management will need to carefully consider whether a yearly tree cover growth of 3% is a desirable goal, or if the trade-off is too steep given the reduction in crops and settlement areas needed to achieve it. While increasing tree cover can be beneficial for the environment, it comes at the cost of important land use for citizens, agriculture, and infrastructure. However, with the implementation of the simulation function, there is flexibility to experiment with different scenarios and values. This allows for informed decision-making on how to best balance tree cover growth with other land use priorities.
...
The **gap statistic** was used to determine the optimal number of clusters for each value of $\alpha$ by comparing the within-cluster variance for a given number of clusters to that of a reference distribution, helping to find the number of clusters that best fits the data without overfitting. To determine gap statistic we used `clusGap` function[@clusgapCite] from `cluster` [@clusterCite] package. Finally function `pam` [@pamCite] from `cluster` [@clusterCite] package was employed to carry out the actual assignment of observations to clusters.
...
![Tree Percentage Trends by each cluster when clustering only based on **only** tree percentage variable with $\alpha = 0.5$.](Images/alpha05trend.png)
...
|                              |                              |                              |
|------------------------|------------------------|------------------------|
| ![](Images/parcoord2018.png) | ![](Images/parcoord2019.png) | ![](Images/parcoord2020.png) |
| ![](Images/parcoord2021.png) | ![](Images/parcoord2022.png) | ![](Images/parcoord2023.png) |

: Parallel Coordinate Plot of feature variables used in multi-variable spatio-temporal clustering by year.
