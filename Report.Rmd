...
Through earlier correlation analysis between **tree percentage** and several variables such as **built area percentage** and **crops percentage**, we gained some deep insights and have seen that tree percentage in the most of the Central European areas correlates heavily negative with these variables.
...
We will now compare the four models once again to ensure that XGBoost is indeed the best fitting model. To do this, we can use the R-squared metric, also known as the coefficient of determination, which measures how well the model's predictions align with the actual data.
...
On the other hand, we can also use $RMSE$ (Root Mean Squared Error), to compare models. $RMSE$ measures the average magnitude of the errors between predicted and actual values.
